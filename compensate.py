...
from Devices.ELL14 import ELL14
from Devices.TimeTaggerLmu import TimeTaggerLmu

channels = {
    "H": {"ch": 1, "edge": -1, "trigger": -0.5},
    "V": {"ch": 2, "edge": -1, "trigger": -0.5},
    "P": {"ch": 3, "edge": -1, "trigger": -0.5},
    "M": {"ch": 4, "edge": -1, "trigger": -0.5},
    "SYC": {"ch": 5, "edge": 1, "trigger": 0.25},
    # "CLK": {"ch": 6, "edge": 1, "trigger": 0.25},
}
...
if __name__ == "__main__":
    ...
    if not folder_exists:
        ...
        timestamp = TimeTaggerLmu(channels)

        _, counts = mm.measure_Coupling_Efficiency(source, waveplates, timestamp)
    ...
    if not folder_exists:
        # Do this every Time
        bins, counts = mm.measure_Polcomp_Counts(
            source, waveplates, timestamp, send_pol
        )
    ...
    print("Compensation angles are:\n{}\nwith a loss of {}".format(angles, loss))
