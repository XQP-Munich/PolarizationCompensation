from PolarizationCompensation.Theory import (
    my_sin,
    fit_my_sin,
    fit_Q_Channel,
    angle_between_stokes_vectors,
)
import numpy as np
...
def measure_Coupling_Efficiency(source, waveplates, timestamp):
    # Measure
    angle_max = 90
    deg_per_step = 10
    measure_time = 2  # in s
    ...
    angles = np.arange(0, angle_max, deg_per_step)
    counts_per_second = []

    for angle in angles:
        waveplates.move_like_HWP(angle)
        counts_per_second.append(
            np.mean(timestamp.get_counts_per_second(measure_time)[1], axis=0)
        )

    return angles, counts_per_second


def evaluate_Coupling_Efficiency(
    counts, inv_symbol_map=["H", "V", "P", "M"], folder="./"
):
    counts = np.array(
        [
            [
    ...
    for i in range(4):
        ...
        ps.append(p)
        plt.plot(xs, my_sin(xs, *p), linestyle="dashed", color="dimgrey", linewidth=2)
...
def measure_Polcomp_Counts(source, waveplates, timestamp, send_pol=["V", "M"]):
    ...
    times, counts = timestamp.get_counts_per_second(measurementTime)
...
def evaluate_Polcomp_Counts(
    counts, coupling, sent_pol=["V", "M"], waveplate_setup=["QWP", "HWP", "QWP"]
):
    print(counts)
    ...
    v1 = [
        (matrix[0][0] - matrix[0][1]) / (matrix[0][0] + matrix[0][1]),
        (matrix[0][3] - matrix[0][2]) / (matrix[0][3] + matrix[0][2]),
        (matrix[0][4] - matrix[0][5]) / (matrix[0][4] + matrix[0][5]),
    ]
    v2 = [
        (matrix[1][0] - matrix[1][1]) / (matrix[1][0] + matrix[1][1]),
    ...
    print(
        "Angle between the stokes vectors: {} for:\n{}:{}\n{}:{}".format(
